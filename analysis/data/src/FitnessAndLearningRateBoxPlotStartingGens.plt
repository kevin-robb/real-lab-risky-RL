...
do for [treatment_case in "nurt nonnurt"] {
		...
		#	eval(fstr(i))
		#	eval(fitstr(i))
			stats filename(i*5) using 2:5 #name prefix(i) nooutput
		...
		plotstr = "plot "
		do for [i=0:num_files-1] {
			plotstr = plotstr.sprintf("filename(%d*5) using (x_mean[%d+1]):5 linestyle %d+1 title 'Gen %d'%s", i, i, i, i*5, (i == num_files) ? "" : ", ")
		}
